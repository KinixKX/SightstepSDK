...
float3 calculateEmissionNew(in float3 baseColor, inout float4 finalColor)
{
    ...
    float3 emissionColor1 = 0;
        ...
        float glowInTheDarkMultiplier1 = calculateGlowInTheDark(float(0), float(1), float(1), float(0), float(0), float(0));
            ...
            if (!float(0))
            {
                emissionColor1 = POI2D_SAMPLER_PAN(_EmissionMap1, _MainTex, poiMesh.uv[float(0)], float4(0,0,0,0)) * lerp(1, baseColor, float(0)).rgb * _EmissionColor1.rgb;
            }
            else
            {
                emissionColor1 = UNITY_SAMPLE_TEX2D_SAMPLER(_EmissionMap1, _MainTex, ((.5 + poiLight.nDotV * .5) * float4(1,1,0,0).xy) + _Time.x * float(5)).rgb * lerp(1, baseColor, float(0)).rgb * _EmissionColor1.rgb;
            }
        #else
            emissionColor1 = lerp(1, baseColor, float(0)).rgb * _EmissionColor1.rgb;;
        #endif
        ...
        if (float(1))
        {
            ...
            {
                #if defined(PROP_EMISSIONSCROLLINGCURVE1) || !defined(OPTIMIZER_ENABLED)
                    emissionStrength1 *= UNITY_SAMPLE_TEX2D_SAMPLER(_EmissionScrollingCurve1, _MainTex, TRANSFORM_TEX(poiMesh.uv[float(0)], _EmissionScrollingCurve1) + (dot(pos1, float4(0,-10,0,0)) * float(4.4)) + _Time.x * float(4.12));
                #endif
            }
            else
            {
                emissionStrength1 *= calculateScrollingEmission(float4(0,-10,0,0), float(4.12), float(4.4), float(4.41), float(0), pos1);
            }
        }
