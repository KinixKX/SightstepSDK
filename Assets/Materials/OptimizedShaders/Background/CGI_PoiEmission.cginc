...
{
    float3 emission0 = 0;
    float emissionStrength0 = float(1.14);
    ...
    float glowInTheDarkMultiplier0 = calculateGlowInTheDark(float(0), float(1), float(1), float(0), float(0), float(0));
        ...
        if (!float(0))
        {
            emissionColor0 = POI2D_SAMPLER_PAN(_EmissionMap, _MainTex, poiMesh.uv[float(0)], float4(0,0,0,0)).rgb * lerp(1, baseColor, float(0)).rgb * float4(0.06174221,0.5019608,0,1).rgb;
        }
        else
        {
            emissionColor0 = UNITY_SAMPLE_TEX2D_SAMPLER(_EmissionMap, _MainTex, ((.5 + poiLight.nDotV * .5) * float4(1,1,0,0).xy) + _Time.x * float(5)).rgb * lerp(1, baseColor, float(0)).rgb * float4(0.06174221,0.5019608,0,1).rgb;
        }
    #else
        emissionColor0 = lerp(1, baseColor, float(0)).rgb * float4(0.06174221,0.5019608,0,1).rgb;
    #endif
    ...
    if (float(1))
    {
        ...
        {
            emissionStrength0 *= calculateScrollingEmission(float4(0,10,0,0).xyz, float(5), float(10), float(60), float(0), pos);
        }
    }
    ...
    emissionColor0 = hueShift(emissionColor0, frac(float(0.106) + float(0) * _Time.x) * float(0));
    #if defined(PROP_EMISSIONMASK) || !defined(OPTIMIZER_ENABLED)
