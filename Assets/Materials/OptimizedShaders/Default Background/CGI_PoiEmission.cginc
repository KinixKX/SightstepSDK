...
float3 calculateEmissionNew(in float3 baseColor, inout float4 finalColor)
{
    ...
    float glowInTheDarkMultiplier0 = calculateGlowInTheDark(float(0), float(1), float(1), float(0), float(0), float(0));
        ...
        if (!float(0))
        {
            emissionColor0 = POI2D_SAMPLER_PAN(_EmissionMap, _MainTex, poiMesh.uv[float(0)], float4(0.25,1,0,0)).rgb * lerp(1, baseColor, float(0)).rgb * float4(0.1254902,1.443137,3.011765,1).rgb;
        }
        else
        {
            emissionColor0 = UNITY_SAMPLE_TEX2D_SAMPLER(_EmissionMap, _MainTex, ((.5 + poiLight.nDotV * .5) * float4(5,5,0,0).xy) + _Time.x * float(1)).rgb * lerp(1, baseColor, float(0)).rgb * float4(0.1254902,1.443137,3.011765,1).rgb;
        }
    #else
        emissionColor0 = lerp(1, baseColor, float(0)).rgb * float4(0.1254902,1.443137,3.011765,1).rgb;
    #endif
    ...
    if (float(0))
    {
        ...
        {
            #if defined(PROP_EMISSIONSCROLLINGCURVE) || !defined(OPTIMIZER_ENABLED)
                emissionStrength0 *= UNITY_SAMPLE_TEX2D_SAMPLER(_EmissionScrollingCurve, _MainTex, TRANSFORM_TEX(poiMesh.uv[float(0)], _EmissionScrollingCurve) + (dot(pos, float4(0,0,-5,0).xyz) * float(15)) + _Time.x * float(0)).r;
            #endif
        }
        else
        {
            emissionStrength0 *= calculateScrollingEmission(float4(0,0,-5,0).xyz, float(0), float(15), float(-0.4), float(0), pos);
        }
    }
    
    if (float(0))
    {
        emissionStrength0 *= calculateBlinkingEmission(float(0.2), float(0.6), float(0.5), float(0));
    }
    ...
    float3 emissionColor1 = 0;
        ...
        float glowInTheDarkMultiplier1 = calculateGlowInTheDark(float(0), float(1), float(1), float(0), float(0), float(0));
            ...
            if (!float(0))
            {
                emissionColor1 = POI2D_SAMPLER_PAN(_EmissionMap1, _MainTex, poiMesh.uv[float(0)], float4(0.25,1,0,0)) * lerp(1, baseColor, float(0)).rgb * float4(0.6132076,0.6132076,0.6132076,1).rgb;
            }
            else
            {
                emissionColor1 = UNITY_SAMPLE_TEX2D_SAMPLER(_EmissionMap1, _MainTex, ((.5 + poiLight.nDotV * .5) * float4(5,5,0,1).xy) + _Time.x * float(5)).rgb * lerp(1, baseColor, float(0)).rgb * float4(0.6132076,0.6132076,0.6132076,1).rgb;
            }
        #else
            emissionColor1 = lerp(1, baseColor, float(0)).rgb * float4(0.6132076,0.6132076,0.6132076,1).rgb;;
        #endif
        ...
        if (float(1))
        {
            emissionStrength1 *= calculateBlinkingEmission(float(0.2), float(0.5), float(1), float(0));
        }
        emissionColor1 = hueShift(emissionColor1, frac(float(0) + float(0) * _Time.x) * float(0));
        #if defined(PROP_EMISSIONMASK1) || !defined(OPTIMIZER_ENABLED)
            float emissionMask1 = UNITY_SAMPLE_TEX2D_SAMPLER(_EmissionMask1, _MainTex, TRANSFORM_TEX(poiMesh.uv[float(0)], _EmissionMask1) + _Time.x * float4(0.25,0,0,0));
        #else
