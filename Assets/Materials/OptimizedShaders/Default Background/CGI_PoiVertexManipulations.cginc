    ...
    {
        #if defined(PROP_VERTEXMANIPULATIONHEIGHTMASK) || !defined(OPTIMIZER_ENABLED)
            float3 heightOffset = (tex2Dlod(_VertexManipulationHeightMask, float4(TRANSFORM_TEX(uv, _VertexManipulationHeightMask) + float4(1,1,0,0) * _Time.x, 0, 0)).r - float(0)) * float(1) * worldNormal;
        #else
            float3 heightOffset = float(1) * worldNormal;
        #endif
